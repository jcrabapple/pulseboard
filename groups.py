...
def build_group_summaries(
    ...
    """
    by_status: dict[str, dict[str, Status]] = {}
    if results:
        for r in results:
            by_status.setdefault(r.service_name, {})[r.service_name] = r.status
...
def apply_dependency_impact(
    ...
    for svc in services:
        ...
        for fd in failing_deps:
            if fd["status"] == Status.DEGRADED and worst_dep == Status.DOWN:
                # Only degrade to DEGRADED if worst dep is merely degraded.
                worst_dep = Status.DEGRADED
            elif fd["status"] == Status.DOWN:
                worst_dep = Status.DOWN
