...
### Unreleased
- HTTP checks now send a `User-Agent: PulseBoard/<version>` header by default. Target services can use this to identify the monitor in access logs, distinguish it from random scrapers, and write firewall / rate-limit rules keyed to a single stable string. A user-configured `User-Agent` in the service `headers:` block takes precedence and is never clobbered
- Fail-fast config validation for the `ssl_expiry_warning_days` field on SSL services: negative values, non-integer strings (e.g. `"two weeks"`), and booleans are now rejected at config-load time with a clear `ConfigError` naming the service, instead of silently disabling the cert-expiry warning window or crashing the SSL checker at runtime with an opaque `TypeError` during the `days_left <= ssl_expiry_warning_days` comparison. `0` is accepted (it means DEGRADED is only set once the certificate has actually expired) and the default of 14 days is preserved
- Fail-fast config validation for the `body_regex` field: an unbalanced character class, unclosed group, or otherwise non-compilable Python regex is now rejected at config-load time with a clear `ConfigError` naming the service and the offending pattern, instead of failing later inside the HTTP content checker with an opaque `re.error`
